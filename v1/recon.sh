...
join.py out-of-scope | sed 's/,/|/g' | xargs -n1 -I{} sh -c "cat domains | grep -avE '^({})$'" > t
rm domains
mv t domains

#get third-level-domains
echo '[*]Generating third-level-domains'
3levels.py domains | sort -u > t
sanitizer.py root t > third-level-domains
rm t

#Send third-level-domains to bounty4
splitfam third-level-domains
send_data third-level-domains
rm third-level-domains
mv tempf third-level-domains

#Discovery on third-level-domains
echo '[*]Starting discovery on third-level-domains'
subs third-level-domains-2
rm third-level-domains-2
check_files
